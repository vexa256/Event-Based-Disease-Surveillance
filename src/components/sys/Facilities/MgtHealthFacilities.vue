  ...
  <div>
    ...
    <div class="card-body px-5 py-5 bg-light shadow-lg table-responsive">
      <form method="POST" id="AddHF">
        ...
        <input type="text" name="HFID" class="randomid d-none" />
      </form>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      records: [],
      titleColumnName: "HealthFacilityName",
      ignoredColumns: [
        "created_at",
        "updated_at",
        "ProvinceID",
  ...
  mounted() {
    ...
    window.SetMyPageTitle("Create and Manage HFs");
    ...
    window.FormEngine(this.tableName, this.excludedColumns, "#MgtHF", 12, 12);
  ...
  methods: {
    ...
    async fetchAndDisplayData() {
      try {
        const {
          data: { records },
        } = await axios.post(`${this.SERVER_URL}FetchHealthFacilities`);
