    ...
    <div class="card-body px-5 py-5 bg-light shadow-lg table-responsive">
      <form method="POST" id="AddEF">
        ...
        <input
          type="text"
          name="TableName"
          value="environment_facilities"
          class="d-none"
        />
        <input type="text" name="created_at" value="" class="d-none" />
        <input type="text" name="EFID" class="randomid d-none" />
      </form>
    </div>
          ...
          <div class="modal-header shadow-lg">
            <div class="col-12">
              <input
                ...
                v-model="searchTerm"
                placeholder="Search Environmental Facilities"
              />
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      provinceId: null,
      searchTerm: "",
      tableName: "environment_facilities",
      excludedColumns: [
        "created_at",
        "updated_at",
        "ProvinceID",
      ...
      records: [],
      titleColumnName: "EnvironmentalFacilityName",
      ignoredColumns: [
        "created_at",
        "updated_at",
        "ProvinceID",
  ...
  mounted() {
    ...
    window.SetMyPageTitle("Create and Manage Environmental Facilities");
    ...
    setRandomId();
    setCreatedAtValue();
    SendFormEngine("AddEF", () => this.fetchAndDisplayData());
    ...
    window.FormEngine(this.tableName, this.excludedColumns, "#MgtEF", 12, 12);
  ...
  methods: {
    ...
    async fetchAndDisplayData() {
      try {
        const {
          data: { records },
        } = await axios.post(`${this.SERVER_URL}FetchEnvFacilities`);
