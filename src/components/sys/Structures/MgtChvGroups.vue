  ...
  <div>
    ...
    <div class="card-body px-5 py-5 bg-light shadow-lg table-responsive">
      <form method="POST" id="AddCHVGroup">
        ...
        <div id="MgtCHVGroups"></div>
        <input type="text" name="PostRoute" value="MassInsert" class="d-none" />
        <input type="text" name="TableName" value="chv_groups" class="d-none" />
        <input type="text" name="created_at" value="" class="d-none" />
        <input type="text" name="ChvGroupID" class="randomid d-none" />
      </form>
    </div>
...
export default {
  ...
  mounted() {
    ...
    window.SetMyPageTitle("Create and Manage CHV Groups");
    ...
    setRandomId();
    setCreatedAtValue();
    SendFormEngine("AddCHVGroup", () => this.fetchAndDisplayData());
  ...
  methods: {
    ...
    async fetchAndDisplayData() {
      try {
        const {
          data: { records },
        } = await axios.post(`${this.SERVER_URL}FetchChvGroups`);
