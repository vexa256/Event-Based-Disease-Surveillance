<template>
  <div>
    <div class="card-body mb-3 pb-3 pe-2 bg-light shadow-lg table-responsive">
      ...
      >
        <i class="fas fa-eye"></i> View Provinces
      </button>
    </div>
    <div class="card-body px-5 py-5 bg-light shadow-lg table-responsive">
      <form method="POST" id="AddProvince">
...
export default {
  data() {
    return {
      ...
      provinceId: null,
      tableName: "provinces",
      excludedColumns: ["created_at", "updated_at", "ProvinceID"],
      records: [],
  ...
  methods: {
    ...
    async fetchAndDisplayData() {
      try {
        const {
          data: { records },
        } = await axios.post(`${this.SERVER_URL}FetchProvinces`, {
