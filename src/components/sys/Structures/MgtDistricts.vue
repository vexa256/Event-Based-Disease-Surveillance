<template>
  <div>
    ...
    <div class="card-body px-5 py-5 bg-light shadow-lg table-responsive">
      <form method="POST" id="AddDistricts">
        <div class="mb-3 col-md-12 my-2 py-2">
          <label id="label" for="" class="required form-label my-2 py-2"
            >Select Parent Province</label
          >
          <select
            required
            id="province-select"
        ...
        <div id="MgtDistricts"></div>
        <input type="text" name="PostRoute" value="MassInsert" class="d-none" />
        <input type="text" name="TableName" value="districts" class="d-none" />
    ...
    >
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <form action="#" id="ThisUpdatesProvinces">
            <div class="modal-body">
                ...
                <div id="UpdateProvinceForm">
                  ...
                  <div class="mb-3 col-md-12 my-2 py-2">
                    <label
                      id="label"
                      for=""
                      class="required form-label my-2 py-2"
                      >Select Parent Province</label
                    >
                    <select
                      required
...
export default {
  data() {
    return {
      ...
      searchTerm: "",
      tableName: "districts",
      excludedColumns: ["created_at", "updated_at", "ProvinceID", "DistrictID"],
      records: [],
      titleColumnName: "DistrictName",
      ignoredColumns: ["created_at", "updated_at", "ProvinceID", "DistrictID"],
  ...
  mounted() {
    ...
    window.SetMyPageTitle("Create and Manage Districts");
    ...
    setRandomId();
    setCreatedAtValue();
    SendFormEngine("AddDistricts", () => this.fetchAndDisplayData());
    SendFormEngine("ThisUpdatesProvinces", () => this.fetchAndDisplayData());
  ...
  methods: {
    async FetchProvinces() {
      console.log(window.SERVER_URL + "FetchProvinces");

      try {
        const response = await axios.post(
          `${window.SERVER_URL}FetchProvinces`,
          {
            excludedColumns: this.ignoredColumns,
          }
        ); // Replace 'column1', 'column2' with the actual column names you wish to exclude
        if (response.data && response.data.records) {
          console.log(response.data.records);
          this.provinces = response.data.records;
        }
      } catch (error) {
    ...
    async fetchAndDisplayData() {
      try {
        const {
          data: { records },
        } = await axios.post(`${this.SERVER_URL}FetchDistricts`);
