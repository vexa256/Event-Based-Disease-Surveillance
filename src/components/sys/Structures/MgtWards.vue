  ...
  <div>
    ...
    <div class="card-body px-5 py-5 bg-light shadow-lg table-responsive">
      <form method="POST" id="AddWard">
        ...
        <input type="text" name="TableName" value="wards" class="d-none" />
        <input type="text" name="created_at" value="" class="d-none" />
        <input type="text" name="WardID" class="randomid d-none" />
      </form>
    </div>
...
export default {
  ...
  mounted() {
    ...
    setRandomId();
    setCreatedAtValue();
    SendFormEngine("AddWard", () => this.fetchAndDisplayData());
  ...
  methods: {
    ...
    async fetchAndDisplayData() {
      try {
        const {
          data: { records },
        } = await axios.post(`${this.SERVER_URL}FetchWards`);
