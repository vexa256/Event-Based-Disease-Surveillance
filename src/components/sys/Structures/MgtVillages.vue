  ...
  <div>
    ...
    <div class="card-body px-5 py-5 bg-light shadow-lg table-responsive">
      <form method="POST" id="AddVillage">
        ...
        <div id="MgtVillages"></div>
        <input type="text" name="PostRoute" value="MassInsert" class="d-none" />
        <input type="text" name="TableName" value="villages" class="d-none" />
        <input type="text" name="created_at" value="" class="d-none" />
        <input type="text" name="VillageID" class="randomid d-none" />
      </form>
    </div>
...
export default {
  ...
  mounted() {
    ...
    window.SetMyPageTitle("Create and Manage villages");
    ...
    setRandomId();
    setCreatedAtValue();
    SendFormEngine("AddVillage", () => this.fetchAndDisplayData());
    SendFormEngine("UpdateFormData", () => this.fetchAndDisplayData());
    window.removeElementsWithXIdClass();

    window.FetchSelect("village-select", "FetchWards", "WardName", "WardID");
  ...
  methods: {
    ...
    async fetchAndDisplayData() {
      try {
        const {
          data: { records },
        } = await axios.post(`${this.SERVER_URL}FetchVillages`);
