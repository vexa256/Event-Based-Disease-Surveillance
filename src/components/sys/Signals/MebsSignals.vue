<template>
    <div>
        <div class="card-body mb-3 pb-3 pe-2 bg-light shadow-lg table-responsive">
            <router-link to="/ViewMebsSignals">
                <button type="button" class="btn btn-sm float-end mt-3 btn-danger ms-2">
                    <i class="fas fa-eye"></i> View MEBS Signals
                </button>
        ...
        <div class="card-body px-5 py-5 bg-light shadow-lg table-responsive">
            <form method="POST" id="AddSignalCategory" enctype="multipart/form-data">
                ...
                <input type="text" name="EbsType" value="MEBS" class="d-none" />
...
export default {
    ...
    mounted() {
        ...
        window.SetMyPageTitle("Create and Manage MEBS signals");
    ...
    methods: {
        ...
        async fetchAndDisplayData() {
            try {
                const {
                    data: { records },
                } = await axios.post(`${this.SERVER_URL}FetchMEBsSignals`, {
